...
namespace DSENT
{
    ...
    void DSENT::run(int argc_, char** argv_, ostream& ost_)
    {    
        // Initialize DSENT framework (setup log file, config file, ...)
        initialize(argc_, argv_, ost_);
    ...
    void DSENT::initialize(int argc_, char** argv_, ostream& ost_)
    {
        ...
        {
            ModelGen::printAvailableModels(ost_);
            exit(0);
        }
    ...
    void DSENT::buildModel(ostream& ost_)
    {
        Config* dsent_config = Config::getSingleton();
    ...
    void DSENT::processQuery(ostream& ost_)
    {
        Config* dsent_config = Config::getSingleton();
        ...
        for(unsigned int i = 0; i < queries.size(); ++i)
        {
            ...
            if(ms_is_verbose_)
            {
                ...
                ost_ << str << endl;
                ost_ << String(str.size(), '-') << endl;
            }

            processQuery(curr_query, true, ost_);
    ...
    const void* DSENT::processQuery(const String& query_str_, bool is_print_, ostream& ost_)
    {
        ...
        else
        {
            ...
            if(is_print_)
            {
                result->print(query_type + Model::TYPE_SEPARATOR + query_hier + 
                        Model::SUBFIELD_SEPARATOR + query_subfield, query_detail, ost_);
            }
        }
        return query_result;
    }
    ...
    void DSENT::reportTiming(ostream& ost_)
    {
        Config* dsent_config = Config::getSingleton();
        ...
        ost_ << "Report timing:" << endl;
        ost_ << "==============" << endl;
        for(unsigned int i = 0; i < start_net_names.size(); ++i)
        {
            const String& net_name = start_net_names[i];
            double timing = timing_tree.performCritPathExtract(electrical_model->getNet(net_name));
            ost_ << net_name << " = " << timing << endl;
        }
        ost_ << "==============" << endl;
        return;
    }

    void DSENT::processEvaluate(ostream& ost_)
    {
        Config* dsent_config = Config::getSingleton();
        ...
        calc.evaluateString(eval_str, ost_);
    ...
    double DSENT::DSENTCalculator::getEnvVar(const String& var_name_) const
    {
        ...
        {
            // Wish there was a way to not have to pass in a stream if we aren't doing anything with it
            const Result* result = (const Result*)DSENT::processQuery(var_name_ + "@0", false, cout);
